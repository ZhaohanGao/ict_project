...
# 如果当前状态是“视频检测界面”，优先显示并中断其他内容
if st.session_state.show_detection_page:
    st.subheader("🚦Video Detection Interface")

    if st.button("🔙Back to Dashboard"):
        st.session_state.show_detection_page = False
        st.rerun()

    camera_num_str = st.text_input("Camera ID(index)", value="1")
    # 校验输入为数字并格式化为 'CAMxxx'
    ...
    if st.button("Start detection") and video_file is not None and camera_id:
        with st.spinner("Uploading and dectecting，please wait..."):
            files = {
                "video": video_file,
...
with col1:
    ...
    if st.session_state.selected_incident is not None:
        ...
        # 🚀 添加显眼按钮：从事件详情跳转到视频检测界面
        st.markdown("---")
        if not st.session_state.show_detection_page:
            if st.button("🚀 进入视频检测界面 / Go to Video Detection"):
                st.session_state.show_detection_page = True
                st.rerun()
